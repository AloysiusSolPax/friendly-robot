...
var PERSONAL_MILESTONES=[{hours:10,label:"Ten Hours In",icon:"🌱"},{hours:25,label:"Committed",icon:"🌿"},{hours:50,label:"Dedicated",icon:"🌾"},{hours:100,label:"Century",icon:"⭐"},{hours:200,label:"Farm Soul",icon:"🏆"},{hours:500,label:"Legend",icon:"🌟"}];
...
function Main(props){
  ...
  async function addOb(){if(!oInp.trim())return;setOLd(true);try{var raw=await aiCall("Farm observation. ONLY JSON:{\"text\":\"t\",\"category\":\"Crops|Weather|Equipment|Volunteers|Animals|General\",\"suggestedTask\":\"task or null\"}. Obs:\""+oInp+"\"");var p=JSON.parse(raw.replace(/```json|```/g,"").trim());setObs(function(pv){return pv.concat([{id:mkid(),text:p.text||oInp.trim(),category:p.category||"General",date:todayStr,suggestedTask:p.suggestedTask||null}]);});}catch(e){setObs(function(pv){return pv.concat([{id:mkid(),text:oInp.trim(),category:"General",date:todayStr}]);});}setOInp("");setOLd(false);}
  async function buildTech(){if(!tR.trim())return;setTLd(true);try{var raw=await aiCall("Farm technique card. Crop:"+tCr+" Task:"+tTk+" Notes:\""+tR+"\". ONLY JSON:{\"title\":\"t\",\"steps\":[\"s\"],\"tips\":\"tip\"}");var p=JSON.parse(raw.replace(/```json|```/g,"").trim());setTechs(function(pv){return pv.concat([{id:mkid(),title:p.title,steps:p.steps,tips:p.tips}]);});setTR("");setTSt("Created!");}catch(e){setTSt("Error");}setTLd(false);setTimeout(function(){setTSt("");},3000);}
  function saveHv(){var am=parseFloat(hAm);if(!am)return;setHvs(function(pv){return pv.concat([{id:mkid(),session:hSe,label:dLbl(hSe,ss),crop:hCr,amount:am,unit:hUn,quality:hQu,date:todayStr}]);});setHAm("");}
  function saveAtt(){
          ...
          <div>
            <div style={{background:"linear-gradient(135deg,#fce8d0,#f8dcc0)",borderRadius:24,padding:26,marginBottom:18,position:"relative",overflow:"hidden",boxShadow:T.shadowLg}}>
              ...
              <div style={{display:"flex",gap:20,alignItems:"center",justifyContent:"center"}}>
                {myWD&&<div style={{textAlign:"center"}}><div style={{fontSize:38}}>{wIc(myWD.weatherCode||0)}</div><div style={{fontSize:30,fontWeight:300,color:T.text,lineHeight:1}}>{myWD.tempF}°</div><div style={{fontSize:12,color:T.textMid,marginTop:2}}>{myWD.condition}</div></div>}
          ...
          return(
            <div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:18,flexWrap:"wrap"}}>
                {[["harvest","Harvest"],["attendance","Attendance"],["tasks","Tasks"],["crops","By Crop"],["season","Season"],["badges","Badges"]].map(function(arr){
