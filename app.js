          ...
          <div>
            <div style={{background:SPcard,borderRadius:20,padding:18,marginBottom:16,border:"1px solid "+SPbdr}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                {[{n:"Kale",ic:"🥬"},{n:"Collards",ic:"🥬"},{n:"Spinach",ic:"🌿"},{n:"Arugula",ic:"🌱"},{n:"Sorrel",ic:"🌿"},{n:"Chard",ic:"🥬"},{n:"Lettuce",ic:"🥗"},{n:"Cabbage",ic:"🥬"},{n:"Broccoli",ic:"🥦"},{n:"Cauliflower",ic:"🥦"},{n:"Kohlrabi",ic:"🥦"},{n:"Strawberries",ic:"🍓"},{n:"Tomatoes",ic:"🍅"},{n:"Peppers",ic:"🫑"},{n:"Eggplant",ic:"🍆"},{n:"Cucumbers",ic:"🥒"},{n:"Zucchini",ic:"🥬"},{n:"Squash",ic:"🎃"},{n:"Pumpkins",ic:"🎃"},{n:"Corn",ic:"🌽"},{n:"Beans",ic:"🫘"},{n:"Peas",ic:"🫛"},{n:"Carrots",ic:"🥕"},{n:"Parsnips",ic:"🥕"},{n:"Beets",ic:"🟣"},{n:"Turnips",ic:"🫛"},{n:"Radishes",ic:"🌸"},{n:"Sunchokes",ic:"🌻"},{n:"Potatoes",ic:"🥔"},{n:"Sweet Potatoes",ic:"🍠"},{n:"Garlic",ic:"🧄"},{n:"Onions",ic:"🧅"},{n:"Leeks",ic:"🧅"},{n:"Fennel",ic:"🌿"},{n:"Celery",ic:"🌿"},{n:"Herbs",ic:"🌾"},{n:"Basil",ic:"🌿"},{n:"Cilantro",ic:"🌿"},{n:"Dill",ic:"🌿"},{n:"Mint",ic:"🌿"},{n:"Sunflowers",ic:"🌻"},{n:"Watermelon",ic:"🍉"}].map(function(crop){
            ...
            {harvestedCrops.length>0&&<div style={{background:SPcard,borderRadius:20,padding:18,border:"1px solid "+SPbdr}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:12}}>
                {[{n:"Kale",ic:"🥬"},{n:"Collards",ic:"🥬"},{n:"Spinach",ic:"🌿"},{n:"Arugula",ic:"🌱"},{n:"Sorrel",ic:"🌿"},{n:"Chard",ic:"🥬"},{n:"Lettuce",ic:"🥗"},{n:"Cabbage",ic:"🥬"},{n:"Broccoli",ic:"🥦"},{n:"Cauliflower",ic:"🥦"},{n:"Kohlrabi",ic:"🥦"},{n:"Strawberries",ic:"🍓"},{n:"Tomatoes",ic:"🍅"},{n:"Peppers",ic:"🫑"},{n:"Eggplant",ic:"🍆"},{n:"Cucumbers",ic:"🥒"},{n:"Zucchini",ic:"🥬"},{n:"Squash",ic:"🎃"},{n:"Pumpkins",ic:"🎃"},{n:"Corn",ic:"🌽"},{n:"Beans",ic:"🫘"},{n:"Peas",ic:"🫛"},{n:"Carrots",ic:"🥕"},{n:"Parsnips",ic:"🥕"},{n:"Beets",ic:"🟣"},{n:"Turnips",ic:"🫛"},{n:"Radishes",ic:"🌸"},{n:"Sunchokes",ic:"🌻"},{n:"Potatoes",ic:"🥔"},{n:"Sweet Potatoes",ic:"🍠"},{n:"Garlic",ic:"🧄"},{n:"Onions",ic:"🧅"},{n:"Leeks",ic:"🧅"},{n:"Fennel",ic:"🌿"},{n:"Celery",ic:"🌿"},{n:"Herbs",ic:"🌾"},{n:"Basil",ic:"🌿"},{n:"Cilantro",ic:"🌿"},{n:"Dill",ic:"🌿"},{n:"Mint",ic:"🌿"},{n:"Sunflowers",ic:"🌻"},{n:"Watermelon",ic:"🍉"}].filter(function(c){return harvestedCrops.includes(c.n);}).map(function(c){return <div key={c.n} style={{background:"linear-gradient(135deg,#1a2a2a,#0f1a1a)",border:"1px solid "+SPteal+"44",borderRadius:16,padding:"12px 10px",textAlign:"center",minWidth:64}}><div style={{fontSize:28}}>{c.ic}</div><div style={{fontSize:10,color:SPteal,marginTop:4,fontWeight:600}}>{c.n}</div></div>;})}
              </div>
              <div style={{fontSize:12,color:SPdim}}>{harvestedCrops.length} of 42 crops harvested</div>
            </div>}
          </div>
        )}
...
function Main(props){
  ...
  var[editId,setEditId]=useState(null); var[editData,setEditData]=useState({});
  ...
  var[techQ,setTechQ]=useState(""); var[techQAns,setTechQAns]=useState(""); var[techQLD,setTechQLD]=useState(false); var[techSearch,setTechSearch]=useState(""); var[techAddOpen,setTechAddOpen]=useState(false);
  ...
  function getVolHours(vid){var v=vols.find(function(x){return x.id===vid;});return v&&v.hoursOverride!=null?v.hoursOverride:getVolSessions(vid)*3;}
  var next=getNext();
  ...
  async function askTechQ(){if(!techQ.trim())return;setTechQLD(true);setTechQAns("");try{var ctx=techs.length>0?"\n\nThis farm's saved guides:\n"+techs.map(function(t){return"- "+t.title+": "+(t.steps||[]).join(" → ")+(t.tips?" Tip: "+t.tips:"");}).join("\n"):"";var raw=await aiCall("You are a helpful farm assistant for Little Portion Farm in Ellicott City, MD (Zone 7a). Answer this farming question clearly and practically in 3-6 sentences."+ctx+"\n\nQuestion: "+techQ);setTechQAns(raw.trim());}catch(e){setTechQAns("Could not answer — check your API key in Settings.");}setTechQLD(false);}
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"20px 16px 60px"}}>
        ...
        {tab==="harvest"&&(
          <div>
            ...
            {hvs.slice().reverse().map(function(h){var cc=gcc(h.crop);var isEd=editId===h.id;return <div key={h.id} style={crd()}><div style={{height:2,background:cc.ac}}/><div style={{padding:"12px 16px"}}>{isEd?<div><div style={{display:"flex",gap:6,marginBottom:8}}><select value={editData.crop||h.crop} onChange={function(e){setEditData(function(d){return Object.assign({},d,{crop:e.target.value});});}} style={Object.assign({},inp_s,{flex:2,padding:"6px 8px",fontSize:12})}>{HV_CROPS.map(function(c){return <option key={c}>{c}</option>;})}</select><input type="number" value={editData.amount!==undefined?editData.amount:h.amount} onChange={function(e){setEditData(function(d){return Object.assign({},d,{amount:e.target.value});});}} style={Object.assign({},inp_s,{flex:1,padding:"6px 8px",fontSize:12})}/><select value={editData.unit||h.unit} onChange={function(e){setEditData(function(d){return Object.assign({},d,{unit:e.target.value});});}} style={Object.assign({},inp_s,{flex:1,padding:"6px 8px",fontSize:12})}>{HV_UNITS.map(function(u){return <option key={u}>{u}</option>;})}</select></div><div style={{display:"flex",gap:6,marginBottom:8}}><select value={editData.quality||h.quality} onChange={function(e){setEditData(function(d){return Object.assign({},d,{quality:e.target.value});});}} style={Object.assign({},inp_s,{flex:1,padding:"6px 8px",fontSize:12})}>{HV_QUAL.map(function(q){return <option key={q}>{q}</option>;})}</select></div><div style={{display:"flex",gap:8}}><button onClick={function(){setHvs(function(pv){return pv.map(function(x){return x.id===h.id?Object.assign({},x,{crop:editData.crop||h.crop,amount:parseFloat(editData.amount)||h.amount,unit:editData.unit||h.unit,quality:editData.quality||h.quality}):x;});});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"8px"})}>Save</button><button onClick={function(){setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"8px"})}>Cancel</button></div></div>:<div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontWeight:700,fontSize:15,color:cc.ac,flex:1}}>{h.crop}</span><span style={{fontSize:20,fontWeight:300,color:T.text}}>{h.amount} <span style={{fontSize:12,color:T.textDim}}>{h.unit}</span></span><button onClick={function(){setEditId(h.id);setEditData({crop:h.crop,amount:h.amount,unit:h.unit,quality:h.quality});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:13,padding:"0 2px"}} title="Edit">✎</button><button onClick={function(){setHvs(function(pv){return pv.filter(function(x){return x.id!==h.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:16}}>X</button></div><div style={{display:"flex",gap:6,marginTop:6}}><Pill bg={cc.bg} color={cc.ac}>{h.quality}</Pill><span style={{fontSize:11,color:T.textDim}}>{h.date}</span></div></div>}{h.photo&&<div style={{position:"relative",marginTop:8}}><img src={h.photo} style={{width:"100%",borderRadius:10,maxHeight:150,objectFit:"cover",display:"block"}}/><button onClick={function(){setHvs(function(pv){return pv.map(function(x){return x.id===h.id?Object.assign({},x,{photo:null}):x;});});}} style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,0.55)",color:"#fff",border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:13,lineHeight:"24px",textAlign:"center",padding:0}}>✕</button></div>}</div></div>;})}
          </div>
        )}
          ...
          <div>
            ...
            {vT==="attendance"&&(
              <div>
                ...
                {Object.entries(att).map(function(entry){var sessId=entry[0];return entry[1].map(function(e){var isEd=editId===e.id;return <div key={e.id} style={crd()}><div style={{padding:"12px 16px",borderBottom:"1px solid "+T.border,display:"flex",alignItems:"center"}}><span style={{fontSize:13,fontWeight:700,color:T.peach,flex:1}}>{e.label}</span><span style={{fontSize:11,color:T.textDim}}>{e.date}</span>{!isEd&&<><button onClick={function(){setEditId(e.id);setEditData({count:String(e.count),signup:String(e.signup),names:e.names||""});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:13,marginLeft:8}} title="Edit">✎</button><button onClick={function(){setAtt(function(pv){var u=Object.assign({},pv);u[sessId]=(u[sessId]||[]).filter(function(x){return x.id!==e.id;});return u;});}} style={{background:"none",border:"none",cursor:"pointer",color:T.rose,fontSize:16,marginLeft:4}} title="Delete">X</button></>}</div><div style={{padding:"12px 16px"}}>{isEd?<div><div style={{display:"flex",gap:8,marginBottom:8}}><div style={{flex:1}}><div style={{fontSize:10,color:T.textDim,marginBottom:4}}>Signed Up</div><input type="number" value={editData.signup} onChange={function(e){setEditData(function(d){return Object.assign({},d,{signup:e.target.value});});}} style={Object.assign({},inp_s,{width:"100%",padding:"6px 8px",fontSize:13})}/></div><div style={{flex:1}}><div style={{fontSize:10,color:T.textDim,marginBottom:4}}>Showed</div><input type="number" value={editData.count} onChange={function(e){setEditData(function(d){return Object.assign({},d,{count:e.target.value});});}} style={Object.assign({},inp_s,{width:"100%",padding:"6px 8px",fontSize:13})}/></div></div><input value={editData.names} onChange={function(e){setEditData(function(d){return Object.assign({},d,{names:e.target.value});});}} placeholder="Volunteer names..." style={Object.assign({},inp_s,{width:"100%",padding:"6px 8px",fontSize:12,marginBottom:8})}/><div style={{display:"flex",gap:8}}><button onClick={function(){var nc=parseInt(editData.count)||e.count;var ns=parseInt(editData.signup)||e.signup;setAtt(function(pv){var u=Object.assign({},pv);u[sessId]=(u[sessId]||[]).map(function(x){return x.id===e.id?Object.assign({},x,{count:nc,signup:ns,names:editData.names}):x;});return u;});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"8px"})}>Save</button><button onClick={function(){setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"8px"})}>Cancel</button></div></div>:<div><div style={{display:"flex",gap:8}}><Stat label="Signup" value={e.signup} color={T.textMid}/><Stat label="Showed" value={e.count} color={e.count>=e.signup?T.green:T.rose}/><Stat label="Rate" value={(e.signup?Math.round(e.count/e.signup*100):100)+"%"} color={T.peach}/></div>{e.names&&<div style={{marginTop:8,fontSize:12,color:T.textDim,fontStyle:"italic"}}>{e.names}</div>}</div>}</div></div>;});})}
              </div>
            )}
            {vT==="roster"&&(
              <div>
                ...
                {vols.map(function(v){
                  ...
                  return <div key={v.id} style={crd({overflow:"hidden"})}>
                    <div onClick={function(){setVolDetail(function(prev){return prev===v.id?null:v.id;});}} style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
                      ...
                      <button onClick={function(e){e.stopPropagation();if(window.confirm("Remove "+v.name+" from the roster?"))setVols(function(pv){return pv.filter(function(x){return x.id!==v.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:T.rose,fontSize:18,padding:"0 2px",lineHeight:1}} title="Remove volunteer">X</button>
                    </div>
                    {isOpen&&<div style={{borderTop:"1px solid "+T.border,padding:"14px 16px"}}>
                      ...
                      {editId===v.id?<div style={{marginTop:12}}><input value={editData.name||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{name:e.target.value});});}} placeholder="Name" style={Object.assign({},inp_s,{width:"100%",marginBottom:8,padding:"8px 12px"})}/><select value={editData.experience||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{experience:e.target.value});});}} style={Object.assign({},inp_s,{width:"100%",marginBottom:8,padding:"8px 12px"})}>{EXP_LVL.map(function(l){return <option key={l}>{l}</option>;})}</select><div style={{marginBottom:8}}><div style={{fontSize:10,color:T.textDim,marginBottom:4}}>Hours Override <span style={{fontStyle:"italic"}}>(leave blank to use auto: sessions × 3)</span></div><input type="number" min="0" value={editData.hoursOverride} onChange={function(e){setEditData(function(d){return Object.assign({},d,{hoursOverride:e.target.value});});}} placeholder={"Auto: "+getVolHours(v.id)+" hrs"} style={Object.assign({},inp_s,{width:"100%",padding:"8px 12px"})}/></div><textarea value={editData.note||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{note:e.target.value});});}} placeholder="Notes..." rows={2} style={Object.assign({},ta_s,{marginBottom:8})}/><div style={{display:"flex",gap:8}}><button onClick={function(e){e.stopPropagation();var ho=editData.hoursOverride!==""&&editData.hoursOverride!=null?parseFloat(editData.hoursOverride):null;setVols(function(pv){return pv.map(function(x){return x.id===v.id?Object.assign({},x,{name:editData.name||v.name,experience:editData.experience||v.experience,note:editData.note,hoursOverride:ho!=null?ho:undefined}):x;});});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"8px",fontSize:11})}>Save</button><button onClick={function(e){e.stopPropagation();setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"8px",fontSize:11})}>Cancel</button></div></div>:<div style={{display:"flex",gap:8,marginTop:12}}><button onClick={function(e){e.stopPropagation();setEditId(v.id);setEditData({name:v.name,experience:v.experience,note:v.note||"",hoursOverride:v.hoursOverride!=null?String(v.hoursOverride):""});}} style={Object.assign({},btn2(T.teal),{flex:1,fontSize:11})}>Edit Volunteer</button><button onClick={function(e){e.stopPropagation();setVols(function(pv){return pv.filter(function(x){return x.id!==v.id;});});}} style={Object.assign({},btn2(T.rose),{flex:1,fontSize:11})}>Remove</button></div>}
                    </div>}
                  </div>;
                })}
              </div>
          ...
          <div>
            ...
            {briefs.map(function(b){
              ...
              return <div key={b.id} style={crd()}><div style={{height:3,background:sevC.color}}/><div style={{padding:"14px 18px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,flexWrap:"wrap"}}><Pill bg={sevC.bg} color={sevC.color}>{b.severity}</Pill><Pill bg={catC.bg} color={catC.ac}>{b.category||"General"}</Pill><span style={{fontSize:11,color:T.textDim,marginLeft:"auto"}}>{b.date}</span><button onClick={function(){setEditId(b.id);setEditData({text:b.text});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:13,padding:"0 2px"}} title="Edit">✎</button><button onClick={function(){setBriefs(function(pv){return pv.filter(function(x){return x.id!==b.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:16}}>X</button></div>
                {editId===b.id?<div><textarea value={editData.text||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{text:e.target.value});});}} rows={3} style={Object.assign({},ta_s,{marginBottom:6})} autoFocus/><div style={{display:"flex",gap:6,marginBottom:6}}><button onClick={function(){setBriefs(function(pv){return pv.map(function(x){return x.id===b.id?Object.assign({},x,{text:editData.text||b.text}):x;});});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"7px",fontSize:11})}>Save</button><button onClick={function(){setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"7px",fontSize:11})}>Cancel</button></div></div>:<p style={{margin:0,fontSize:14,color:T.text,lineHeight:1.7}}>{b.text}</p>}
        ...
        {tab==="observations"&&(
          <div>
            ...
            {obs.map(function(o){var c=gc(o.category||"General");var isEd=editId===o.id;return <div key={o.id} style={crd()}><div style={{height:2,background:c.ac}}/><div style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"flex-start",gap:10}}><div style={{flex:1}}>{isEd?<div><textarea value={editData.text||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{text:e.target.value});});}} rows={3} style={Object.assign({},ta_s,{marginBottom:6})} autoFocus/><div style={{display:"flex",gap:6}}><button onClick={function(){setObs(function(pv){return pv.map(function(x){return x.id===o.id?Object.assign({},x,{text:editData.text||o.text}):x;});});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"7px",fontSize:11})}>Save</button><button onClick={function(){setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"7px",fontSize:11})}>Cancel</button></div></div>:<div><div style={{fontSize:13,color:T.textMid,lineHeight:1.5}}>{o.text}</div><div style={{marginTop:6,display:"flex",gap:8,flexWrap:"wrap"}}><Pill bg={c.bg} color={c.ac}>{o.category||"General"}</Pill><span style={{fontSize:11,color:T.textDim}}>{o.date}</span></div>{o.suggestedTask&&<div style={{marginTop:8,background:T.tealBg,borderRadius:10,padding:"8px 12px",display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:12,color:T.teal,flex:1}}>Suggested: {o.suggestedTask}</span><button onClick={function(){setTasks(function(pv){return pv.concat([{id:mkid(),text:o.suggestedTask,category:"General",done:false,recurring:false,day:TODAY}]);});setObs(function(pv){return pv.map(function(x){return x.id===o.id?Object.assign({},x,{suggestedTask:null}):x;});});}} style={{background:T.teal,color:"#fff",border:"none",borderRadius:8,padding:"4px 10px",fontSize:11,cursor:"pointer",whiteSpace:"nowrap"}}>+ Add</button></div>}</div>}{o.photo&&<div style={{position:"relative",marginTop:8}}><img src={o.photo} style={{width:"100%",borderRadius:10,maxHeight:180,objectFit:"cover",display:"block"}}/><button onClick={function(){setObs(function(pv){return pv.map(function(x){return x.id===o.id?Object.assign({},x,{photo:null}):x;});});}} style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,0.55)",color:"#fff",border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:13,lineHeight:"24px",textAlign:"center",padding:0}}>✕</button></div>}</div>{!isEd&&<div style={{display:"flex",flexDirection:"column",gap:4}}><button onClick={function(){setEditId(o.id);setEditData({text:o.text});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:13,padding:"0 2px"}} title="Edit">✎</button><button onClick={function(){setObs(function(pv){return pv.filter(function(x){return x.id!==o.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:16}}>X</button></div>}</div></div></div>;})}
          </div>
        )}

        {tab==="techniques"&&(
          <div>
            <div style={{background:"linear-gradient(135deg,#e8f4f0,#dce8f0)",borderRadius:24,padding:20,marginBottom:16,boxShadow:T.shadowLg,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"linear-gradient(90deg,"+T.teal+","+T.lavender+","+T.peach+")"}}/>
              <Sec>Ask a Farming Question</Sec>
              <div style={{fontSize:12,color:T.textDim,marginBottom:10,lineHeight:1.5}}>Not sure how to do something? Ask anything — Claude knows your saved guides.</div>
              <div style={{display:"flex",gap:8,marginBottom:techQAns||techQLD?12:0}}>
                <input value={techQ} onChange={function(e){setTechQ(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")askTechQ();}} placeholder="e.g. How do I harvest kale without damaging it?" style={Object.assign({},inp_s,{flex:1,fontSize:13})}/>
                <button onClick={askTechQ} disabled={techQLD||!techQ.trim()} style={Object.assign({},btn(T.teal,"#fff"),{opacity:(techQLD||!techQ.trim())?0.4:1,padding:"11px 16px"})}>{techQLD?"...":"Ask"}</button>
              </div>
              {techQLD&&<div style={{textAlign:"center",fontSize:13,color:T.teal,fontStyle:"italic",padding:"8px 0"}}>Looking that up...</div>}
              {techQAns&&<div><div style={{background:"rgba(255,255,255,0.7)",borderRadius:14,padding:"14px 16px",marginBottom:8}}><p style={{margin:0,fontSize:14,color:T.textMid,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{techQAns}</p></div><div style={{display:"flex",gap:8}}><button onClick={function(){setTechQAns("");setTechQ("");}} style={btn2(T.textDim)}>Clear</button><button onClick={askTechQ} style={btn2(T.teal)}>Ask Again</button></div></div>}
            </div>
            <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center"}}>
              <input value={techSearch} onChange={function(e){setTechSearch(e.target.value);}} placeholder={"Search "+techs.length+" guide"+(techs.length!==1?"s":"")+"..."} style={Object.assign({},inp_s,{flex:1,fontSize:13})}/>
              <button onClick={function(){setTechAddOpen(function(v){return!v;});}} style={Object.assign({},btn(techAddOpen?T.textDim:undefined,techAddOpen?"#fff":undefined),{padding:"11px 18px",fontSize:13})}>{techAddOpen?"Cancel":"+ New"}</button>
            </div>
            {techAddOpen&&<div style={crd({padding:16})}>
              ...
              <textarea value={tR} onChange={function(e){setTR(e.target.value);}} placeholder="Describe how to do it — AI will format it into a clean step-by-step card..." rows={4} style={ta_s}/>
              ...
              <button onClick={function(){buildTech().then(function(){setTechAddOpen(false);});}} disabled={tLd||!tR.trim()} style={Object.assign({},btn(),{marginTop:10,width:"100%",opacity:(tLd||!tR.trim())?0.4:1})}>{tLd?"Building...":"Build Card"}</button>
            </div>}
            {(function(){
              var filtered=techSearch.trim()?techs.filter(function(t){var q=techSearch.toLowerCase();return t.title.toLowerCase().includes(q)||(t.steps||[]).some(function(s){return s.toLowerCase().includes(q);})||(t.tips||"").toLowerCase().includes(q);}):techs;
              if(techs.length===0)return <div style={{textAlign:"center",color:T.textDim,fontSize:14,fontStyle:"italic",marginTop:20,padding:20}}>No guides yet — build your first card or ask a question above.</div>;
              if(filtered.length===0)return <div style={{textAlign:"center",color:T.textDim,fontSize:13,fontStyle:"italic",padding:16}}>No guides match "{techSearch}"</div>;
              return filtered.map(function(tech){var isEd=editId===tech.id;return <div key={tech.id} style={crd()}><div style={{padding:"14px 16px",borderBottom:"1px solid "+T.border,display:"flex",alignItems:"center"}}><span style={{flex:1,fontWeight:700,fontSize:15,color:T.peach}}>{tech.title}</span>{!isEd&&<button onClick={function(){setEditId(tech.id);setEditData({title:tech.title,steps:(tech.steps||[]).join("\n"),tips:tech.tips||""});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:13,padding:"0 4px"}} title="Edit">✎</button>}<button onClick={function(){setTechs(function(pv){return pv.filter(function(x){return x.id!==tech.id;});});}} style={{background:"none",border:"none",cursor:"pointer",color:T.textDim,fontSize:16}}>X</button></div><div style={{padding:"14px 16px"}}>{isEd?<div><input value={editData.title||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{title:e.target.value});});}} placeholder="Title" style={Object.assign({},inp_s,{width:"100%",marginBottom:8})}/><div style={{fontSize:10,color:T.textDim,marginBottom:4,textTransform:"uppercase"}}>Steps (one per line)</div><textarea value={editData.steps||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{steps:e.target.value});});}} rows={5} style={Object.assign({},ta_s,{marginBottom:8})}/><div style={{fontSize:10,color:T.textDim,marginBottom:4,textTransform:"uppercase"}}>Tip</div><textarea value={editData.tips||""} onChange={function(e){setEditData(function(d){return Object.assign({},d,{tips:e.target.value});});}} rows={2} style={Object.assign({},ta_s,{marginBottom:8})}/><div style={{display:"flex",gap:8}}><button onClick={function(){setTechs(function(pv){return pv.map(function(x){return x.id===tech.id?Object.assign({},x,{title:editData.title||tech.title,steps:editData.steps.split("\n").filter(function(s){return s.trim();}),tips:editData.tips}):x;});});setEditId(null);}} style={Object.assign({},btn(T.teal,"#fff"),{flex:1,padding:"8px"})}>Save</button><button onClick={function(){setEditId(null);}} style={Object.assign({},btn2(),{flex:1,padding:"8px"})}>Cancel</button></div></div>:<div>{tech.photo&&<div style={{position:"relative",marginBottom:14}}><img src={tech.photo} style={{width:"100%",borderRadius:10,maxHeight:180,objectFit:"cover",display:"block"}}/><button onClick={function(){setTechs(function(pv){return pv.map(function(x){return x.id===tech.id?Object.assign({},x,{photo:null}):x;});});}} style={{position:"absolute",top:6,right:6,background:"rgba(0,0,0,0.55)",color:"#fff",border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:13,lineHeight:"24px",textAlign:"center",padding:0}}>✕</button></div>}{(tech.steps||[]).map(function(step,i){return <div key={i} style={{display:"flex",gap:12,marginBottom:12}}><div style={{width:28,height:28,borderRadius:"50%",background:T.peachBg2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:T.peach,flexShrink:0}}>{i+1}</div><p style={{fontSize:14,color:T.textMid,lineHeight:1.6,margin:0,paddingTop:4}}>{step}</p></div>;})} {tech.tips&&<div style={{background:T.butterBg,borderRadius:12,padding:"12px 14px"}}><span style={{fontSize:10,color:T.gold,fontWeight:700,textTransform:"uppercase"}}>Tip</span><p style={{fontSize:13,color:T.textMid,lineHeight:1.6,margin:"4px 0 0",fontStyle:"italic"}}>{tech.tips}</p></div>}</div>}</div></div>;});
            })()}
          </div>
        )}
